            ...
            if df is not None:
                ...
                with tab1:
                    ...
                    with col1:
                        ...
                        daily_users = df.groupby('date')['userid'].nunique().reset_index()
                ...
                with tab4:
                    ...
                    if not payment_df.empty:
                        ...
                        daily_revenue = payment_df.groupby('date')['amount'].sum().reset_index()
                ...
                with tab7:
                    ...
                    first_activity = df.groupby('userid')['date'].min().reset_index()
                    first_activity.columns = ['userid', 'cohort_date']
                    first_activity['cohort'] = pd.to_datetime(first_activity['cohort_date']).dt.to_period('W')
                    
                    # Calculate retention by cohort
                    cohort_users = first_activity.merge(df[['userid', 'date']], on='userid')
                    cohort_users['cohort'] = cohort_users['cohort'].astype(str)
                    cohort_users['period'] = ((pd.to_datetime(cohort_users['date']) - 
                                              pd.to_datetime(cohort_users['cohort'])).dt.days / 7).astype(int)
                    
                    retention = cohort_users.groupby(['cohort', 'period'])['userid'].nunique().reset_index()
